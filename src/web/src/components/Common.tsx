...
export function StatusBadge({ status }: { status: string }) {
  return (
    <Badge variant="light" color={statusColors[status] ?? 'gray'} tt="none">
...
export function PageHeader({
  ...
  return (
    <Group justify="space-between" align="flex-end" mb="xl" gap="md">
      <div>
        {eyebrow && (
          <Text size="xs" fw={750} c="indigo" tt="uppercase" lts={1.1} mb={5}>
            {eyebrow}
          </Text>
        )}
        <Title order={1} fz={{ base: 27, sm: 32 }} lh={1.15}>
          {title}
        </Title>
        {description && (
          <Text c="dimmed" mt={7} maw={680}>
            {description}
          </Text>
        )}
      </div>
